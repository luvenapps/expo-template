...
const {
  requestPermission,
  revokePermission,
} = require('@/notifications/notificationPlatform.native');

describe('notificationPlatform.native', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('revokePermission', () => {
    ...
    it('handles revoke errors gracefully', async () => {
      revokePushToken.mockRejectedValue(new Error('Revoke failed'));

      await expect(revokePermission()).rejects.toThrow('Revoke failed');
      expect(revokePushToken).toHaveBeenCalled();
    });
  });
});
